...
v3.0.4 closes the default projection governance gap and adds unified grouped sort validation across all providers.

**Governance:** When no explicit `Select` is specified, the system now injects a governed default projection instead of returning all entity fields. Introduces `DefaultProjectionRule`, wildcard expansion at injection time, `GovernanceValidator` startup checks, and role-based auto-projection.
