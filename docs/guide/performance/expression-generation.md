...
However, for **in-memory LINQ-to-Objects** scenarios (no database), expression generation cost becomes more visible (as seen in [Execution Benchmarks](../execution-pipeline) where FlexQuery and handwritten are within milliseconds).
