...
## FlexQueryRequest (Simplified)

`FlexQueryRequest` is a lightweight, opinionated DTO designed for clean, public-facing APIs. It focuses on the most common operations while maintaining a minimal API surface.
...
As a general rule:
1. **Start with `FlexQueryRequest`**. It covers 90% of standard API filtering and paging needs.
...
[HttpGet]
public async Task<IActionResult> Get([FromQuery] FlexQueryRequest request)
{
