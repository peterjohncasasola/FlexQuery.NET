...
- **Relationship query blocks on `include`**: `include=Orders(filter=...;sort=...;take=...)` replaces `FilteredIncludes` with deep, filtered, sorted trees.
...
- **Validation extended** — the v3 rule pipeline gains include-cardinality/having/sort coverage.
...
| `include(...)` blocks (filter/sort/take per branch) | `FilteredIncludes` | [Include](/docs/guides/include) |
...
| `QueryOptions.FilteredIncludes` | `QueryOptions.Includes` (include tree) | Find/replace; see the Include guide for block syntax. |
...
- **EF Core**: include hydration is composed as EF Core filtered includes (the
  `UseSplitQuery` toggle is gone; the provider decides the SQL shape); include blocks
...
- Every include block level is governed by `AllowedIncludes`.
...
3. Replace `FilteredIncludes` usage with `include(...)` blocks.
