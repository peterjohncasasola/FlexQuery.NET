...
For POST endpoints or typed HTTP clients, use [`FlexQueryRequest`](/docs/api-reference/flexquery-request) instead of `FlexQueryParameters`. Convert with `ToQueryOptions()` and pass the resulting `QueryOptions` to `FlexQueryAsync`:
