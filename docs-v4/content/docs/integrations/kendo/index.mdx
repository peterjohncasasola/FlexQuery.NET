...
Kendo's expected response shape is `{ data, total }` — map from the `QueryResult` envelope,
...
<Callout variant="warning" title="Returning QueryResult directly">
  Kendo expects the payload shape <code>&#123; data, total &#125;</code>. Returning the full FlexQuery envelope breaks
  the grid's schema binding — project the two fields explicitly.
</Callout>
