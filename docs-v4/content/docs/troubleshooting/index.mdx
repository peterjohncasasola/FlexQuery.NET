...
**Fix**: add the navigation to `include` (bounded loading via include blocks is then also
...
Each navigation path may be included at most once per query — merging duplicates would make
filter/take/sort ambiguous.

**Fix**: merge the branch options into one include block:

```http
include=Orders(filter=Status:eq:Delivered; take=3)
```
