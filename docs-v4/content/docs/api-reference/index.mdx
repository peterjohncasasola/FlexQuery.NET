...
| Type | Purpose |
|---|---|
| `FlexQueryParameters` | Query-string binding model (`Filter`, `Sort`, `Select`, `Include` (with optional blocks), `GroupBy`, `Having`, `Aggregate`, `Page`, `PageSize`, `IncludeCount`, `Distinct`, `Mode`, `Cursor`, `UseKeysetPagination`). |
...
| `IncludeBuilder` | `Path(path, filter?, configureChildren?, take?, sort?)`. |
