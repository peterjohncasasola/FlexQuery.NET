...
| :--- | :--- | :--- |
| `FlexQueryParameters` | Query-string binding model for GET endpoints. | See [Query Options](/docs/concepts/query-options) for the full property list. |
| `FlexQueryRequest` | Strongly-typed request object for POST bodies and typed clients. | See [FlexQueryRequest](/docs/api-reference/flexquery-request). |
| `QueryOptions` | Parsed options consumed by the pipeline. | See [Query Options](/docs/concepts/query-options). |
| `QueryResult<T>` | Result envelope returned by every execution method. | See [Query Result](/docs/concepts/query-result). |
...
| Type | Key members |
| :--- | :--- |
| `FluentQueryBuilder` | `Filter`, `Sort`, `Select`, `Include`, `Mode`, `GroupBy`, `Aggregate`, `Having`, `Distinct`, `Page`, `UseKeysetPagination`, `DisablePaging`, `Build` |
| `FilterGroupBuilder` | `Equal`, `NotEqual`, `GreaterThan`, `GreaterThanOrEqual`, `LessThan`, `LessThanOrEqual`, `Contains`, `StartsWith`, `EndsWith`, `In`, `NotIn`, `IsNull`, `IsNotNull`, `Between`, `And`, `Or` |
| `FilterBuilder` / `FilterConditionBuilder<TParent>` | `Field`, `And`, `Or`, `Eq`, `Neq`, `Contains`, `StartsWith`, `EndsWith`, `GreaterThan`, `GreaterThanOrEqual`, `LessThan`, `LessThanOrEqual`, `In`, `Between`, `IsNull`, `NotNull`, `Any`, `All`, `Not`, `AndGroup`, `OrGroup` |
| `SortBuilder` | `Ascending`, `Descending` |
| `AggregateBuilder` | `Sum`, `Count`, `Avg`, `Min`, `Max` |
| `IncludeBuilder` | `Path` |
...
| `ApplyIncludes<T>(query, options)` | Include pipeline: plain inclusions plus relationship query blocks. |
