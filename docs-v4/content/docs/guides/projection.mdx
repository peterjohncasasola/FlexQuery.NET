...
| `flat` | collections flatten into leaf rows via `SelectMany`; a query with `Id` **and** a leaf path collapses root values onto leaf rows | single linear path branching only — multiple branches throw |
