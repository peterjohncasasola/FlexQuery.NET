...
Every filter, HAVING condition, and include-block filter in FlexQuery is built from one
...
2. include-block filters (`Orders(take=5;filter=all:Status:eq:Shipped)` -- see [Include](/docs/guides/include)).
