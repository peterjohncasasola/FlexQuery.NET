...
sorting, projection, includes, grouping, aggregates, paging — interchangeably.
...
aggregates, filtered relationship slices, and keyset paging remain DSL/FQL features. The syntax is a
...
| Include | `include=Orders(take=5;filter=Status:eq:Active;sort=OrderDate:desc)` | same, FQL expressions inside `(...)` blocks | `$expand=Orders` (flat paths only) |
...
| Page / PageSize | `page=1&pageSize=20` | same | `$top` / `$skip` |
...
<Callout variant="note" title="MiniOData scope">
  ...
  <code>$expand</code> (flat navigation paths — nested option blocks are not supported).
