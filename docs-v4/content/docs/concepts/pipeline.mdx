...
| Validation |                    governance limits, include paths, aggregates
+------------+
        |
        v
...
- **EF Core**: the composed expression tree hands off to EF's translation — everything
  becomes SQL. Include branches use EF Core filtered includes (`.Include(...)`
...
- **Dapper**: FlexQuery generates the SQL itself — select list (surface-aware, type-map
  ...
  and include-block `take` becomes a server-side ranked/limited child query.
