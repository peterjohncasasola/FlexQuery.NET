    ...
    <div>
      ...
      <section className="border-b border-zinc-200 dark:border-zinc-800">
        <div className="mx-auto max-w-7xl px-4 py-20 text-center sm:px-6 lg:py-24">
          <div className="mx-auto mb-5 inline-flex items-center gap-2 text-xs font-semibold tracking-[0.08em] text-brand-700 uppercase dark:text-brand-400">
          ...
          <div className="mx-auto mt-10 max-w-2xl overflow-hidden rounded-lg border border-zinc-200 text-left dark:border-zinc-800">
            ...
            <pre className="overflow-x-auto bg-white p-4 font-mono text-[13px] leading-6 dark:bg-zinc-950">
              <code className="whitespace-pre-wrap break-all">{`GET /api/customers?filter=Status:eq:Active&sort=LastName:asc&select=Id,FirstName,Email&page=1&pageSize=20`}</code>
            </pre>
          </div>
        </div>
      </section>
        ...
        <div className="grid gap-x-8 sm:grid-cols-2 lg:grid-cols-4">
          {capabilities.map((c) => (
            <Link
              key={c.title}
              href={c.href}
              className="group border-t border-zinc-200 py-5 transition-colors hover:border-brand-500 dark:border-zinc-800 dark:hover:border-brand-600"
            >
      ...
      <section className="border-y border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900/40">
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6">
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            {steps.map((s, i) => (
              <div key={s.title} className="relative rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <div className="grid gap-x-8 sm:grid-cols-2 lg:grid-cols-3">
          {providers.map((p) => (
            <Link
              key={p.name}
              href={p.href}
              className="group flex items-center gap-3 border-t border-zinc-200 py-4 transition-colors hover:border-brand-500 dark:border-zinc-800 dark:hover:border-brand-600"
            >
              <span className="flex h-9 w-9 items-center justify-center rounded-md bg-brand-50 text-brand-600 dark:bg-brand-950/60 dark:text-brand-400">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-24 sm:px-6">
        <div className="rounded-xl bg-zinc-950 px-8 py-12 text-center dark:bg-zinc-100">
          <Boxes className="mx-auto h-8 w-8 text-brand-400 dark:text-brand-600" />
          <h2 className="mt-4 text-2xl font-bold tracking-tight text-white sm:text-3xl dark:text-zinc-950">
            Build your first query in five minutes
          </h2>
          <p className="mx-auto mt-3 max-w-lg text-zinc-400 dark:text-zinc-600">
            Follow the quick-start guide and ship a dynamic query endpoint today.
          </p>
          <Link
            href="/docs/getting-started/first-query"
            className="mt-6 inline-flex items-center gap-2 rounded-md bg-white px-5 py-2.5 text-sm font-semibold text-zinc-950 transition-colors hover:bg-brand-50 dark:bg-zinc-950 dark:text-white dark:hover:bg-zinc-800"
          >
